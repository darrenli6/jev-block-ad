import type { Category, HiddenItem, Settings, VerdictSource } from "../shared/types";
import { fireworksOut } from "./fireworks";
import { snapOut } from "./snap";

export type HideEffect = Settings["hideEffect"];
...
export class Hider {
  ...
  hide(nid: string, el: Element, fp: string, label: Category, summary: string, source: VerdictSource, mode: CollapseMode, effect: HideEffect = "none"): void {
    if (this.entries.has(nid)) return;
    ...
    if (effect === "snap") {
      ...
      void fireworksOut(el, { onCancel: (cancel) => (entry.cancelSnap = cancel), collapse: mode === "display" }).then(finalize);
    } else {
      finalize();
    }
  }
